...
mat_fname_x = pjoin('Results/Iterative_algorithm/Large_x0_error/DMHE/x.mat')
...
y_1 = sio.loadmat('Results/Iterative_algorithm/Large_x0_error/DMHE/y.mat')
y_exp_1 = y_1['ymea']
y_mhe_1 = y_1['ymhe']
...
""" For Centralized MHE """
mat_fname_x_2 = pjoin('Results/Iterative_algorithm/Large_x0_error/CMHE/x.mat')
x_2 = sio.loadmat(mat_fname_x_2)
x_exp_2 = x_2['xmea']
x_mhe_2 = x_2['xmhe']  # no matter dis or central, x_mhe are the same
x_ol_2 = x_2['xol']
x_clean_2 = x_2['xclean']

# mat_fname_y = pjoin(data_dir, 'y.mat')
y_2 = sio.loadmat('Results/Iterative_algorithm/Large_x0_error/CMHE/y.mat')
y_exp_2 = y_2['ymea']
y_mhe_2 = y_2['ymhe']

""" For general parameters """
mat_fname_timePara = pjoin('Results/Iterative_algorithm/Large_x0_error/DMHE/time_parameters.mat')
...
mat_fname_spacePara = pjoin('Results/Iterative_algorithm/Large_x0_error/DMHE/space_parameters.mat')
...
""" For MHE performance - CMHE """
RMSE_X_cmhe = np.sqrt(np.sum((x_exp_2[:,0:Nx] - x_mhe_2[:,0:Nx,0]) ** 2, axis=1) / Nx)
RMSE_P_cmhe = np.sqrt(np.sum((x_exp_2[:,Nx:] - x_mhe_2[:,Nx:,0]) ** 2, axis=1) / NpTotal)

""" For time usage """
time_usage_1 = sio.loadmat('Results/Iterative_algorithm/Large_x0_error/DMHE/timeUsed.mat')
time_solve_1 = time_usage_1['mhe_timeUsedSolve_list']
time_solve_average_1 = np.mean(time_solve_1, axis=0)
time_usage_2 = sio.loadmat('Results/Iterative_algorithm/Large_x0_error/CMHE/timeUsed.mat')
time_solve_2 = time_usage_2['mhe_timeUsedSolve_list']
time_solve_average_2 = np.mean(time_solve_2, axis=0)

print('Time used to solve a DMHE is', time_solve_average_1)
print('Time used to solve a CMHE is', time_solve_average_2)
...
axs[0].plot(Tplot[:end] / DeltaT, x_exp_1[:end, 16+0], 'r-')
# axs[0].plot(Tplot[:end] / DeltaT, x_exp_2[:end, 16+0])
axs[0].plot(Tplot[:end] / DeltaT, x_mhe_timeS[:end, 16+0], 'g--')
axs[0].plot(Tplot[:end] / DeltaT, x_mhe_2[:end, 16+0], 'b:')
# axs[0].plot(Tplot[:end] / DeltaT, x_ekf[:end, 16+0], 'b-.')
# axs[0].plot(Tplot[:end] / DeltaT, x_enkf[:end, 16+0], 'g:', markersize=1)
axs[0].set_xlim(0, end-1)
axs[0].set_ylim(-0.8, -0.15)
axs[0].set_yticks([-0.75, -0.50, -0.25])
axs[0].set_ylabel(r'$h_{1}$ (m)')
# axs[0].grid()

axs[1].plot(Tplot[:end] / DeltaT, x_exp_1[:end, 16+3], 'r-')
# axs[1].plot(Tplot[:end] / DeltaT, x_exp_2[:end, 16+3])
axs[1].plot(Tplot[:end] / DeltaT, x_mhe_timeS[:end, 16+3], 'g--')
axs[1].plot(Tplot[:end] / DeltaT, x_mhe_2[:end, 16+3], 'b:')
# axs[1].plot(Tplot[:end] / DeltaT, x_ekf[:end, 16+3], 'b-.')
# axs[1].plot(Tplot[:end] / DeltaT, x_enkf[:end, 16+3], 'g:')
axs[1].set_xlim(0, end-1)
axs[1].set_ylim(-0.7, -0.25)
axs[1].set_yticks([-0.65, -0.50, -0.35])
axs[1].set_ylabel(r'$h_{4}$ (m)')
# axs[1].grid()

axs[2].plot(Tplot[:end] / DeltaT, x_exp_1[:end, 16+6], 'r-')
# axs[2].plot(Tplot[:end] / DeltaT, x_exp_2[:end, 16+6])
axs[2].plot(Tplot[:end] / DeltaT, x_mhe_timeS[:end, 16+6], 'g--')
axs[2].plot(Tplot[:end] / DeltaT, x_mhe_2[:end, 16+6], 'b:')
# axs[2].plot(Tplot[:end] / DeltaT, x_ekf[:end, 16+6], 'b-.')
# axs[2].plot(Tplot[:end] / DeltaT, x_enkf[:end, 16+6], 'g:')
axs[2].set_xlim(0, end-1)
axs[2].set_ylim(-0.7, -0.3)
axs[2].set_yticks([-0.65, -0.50, -0.35])
axs[2].set_ylabel(r'$h_{7}$ (m)')
# axs[2].grid()

axs[3].plot(Tplot[:end] / DeltaT, x_exp_1[:end, 16+9], 'r-')
# axs[3].plot(Tplot[:end] / DeltaT, x_exp_2[:end, 16+9])
axs[3].plot(Tplot[:end] / DeltaT, x_mhe_timeS[:end, 16+9], 'g--')
axs[3].plot(Tplot[:end] / DeltaT, x_mhe_2[:end, 16+9], 'b:')
# axs[3].plot(Tplot[:end] / DeltaT, x_ekf[:end, 16+9], 'b-.')
# axs[3].plot(Tplot[:end] / DeltaT, x_enkf[:end, 16+9], 'g:')
axs[3].set_xlim(0, end-1)
axs[3].set_ylim(-0.7, -0.3)
axs[3].set_yticks([-0.65, -0.50, -0.35])
axs[3].set_ylabel(r'$h_{10}$ (m)')
# axs[3].grid()

axs[4].plot(Tplot[:end] / DeltaT, x_exp_1[:end, 16+12], 'r-')
# axs[4].plot(Tplot[:end] / DeltaT, x_exp_2[:end, 16+12])
axs[4].plot(Tplot[:end] / DeltaT, x_mhe_timeS[:end, 16+12], 'g--')
axs[4].plot(Tplot[:end] / DeltaT, x_mhe_2[:end, 16+12], 'b:')
# axs[4].plot(Tplot[:end] / DeltaT, x_ekf[:end, 16+12], 'b-.')
# axs[4].plot(Tplot[:end] / DeltaT, x_enkf[:end, 16+12], 'g:')
axs[4].set_xlim(0, end-1)
axs[4].set_ylim(-0.7, -0.3)
axs[4].set_yticks([-0.65, -0.50, -0.35])
axs[4].set_ylabel(r'$h_{13}$ (m)')
# axs[4].grid()
...
f.legend(['EXP', 'DMHE', 'CMHE'], loc=9, ncol=4, frameon=False)
f.savefig('X_Figure_1.pdf')
f.show()

# For subsystem 2 -------------------------------------------------------------------------------
f, axs = plt.subplots(5, 1, sharex=True, figsize=(6.0, 7.0))
axs[0].plot(Tplot[:end] / DeltaT, x_exp_1[:end, Nx//2+16+0], 'r-')
axs[0].plot(Tplot[:end] / DeltaT, x_mhe_timeS[:end, Nx//2+16+0], 'g--')
axs[0].plot(Tplot[:end] / DeltaT, x_mhe_2[:end, Nx//2+16+0, :], 'b:')
...
axs[1].plot(Tplot[:end] / DeltaT, x_exp_1[:end, Nx//2+16+3], 'r-')
axs[1].plot(Tplot[:end] / DeltaT, x_mhe_timeS[:end, Nx//2+16+3], 'g--')
axs[1].plot(Tplot[:end] / DeltaT, x_mhe_2[:end, Nx//2+16+3,:], 'b:')
...
axs[2].plot(Tplot[:end] / DeltaT, x_exp_1[:end, Nx//2+16+6], 'r-')
axs[2].plot(Tplot[:end] / DeltaT, x_mhe_timeS[:end, Nx//2+16+6], 'g--')
axs[2].plot(Tplot[:end] / DeltaT, x_mhe_2[:end, Nx//2+16+6,:], 'b:')
...
axs[3].plot(Tplot[:end] / DeltaT, x_exp_1[:end, Nx//2+16+9], 'r-')
axs[3].plot(Tplot[:end] / DeltaT, x_mhe_timeS[:end, Nx//2+16+9], 'g--')
axs[3].plot(Tplot[:end] / DeltaT, x_mhe_2[:end, Nx//2+16+9,:], 'b:')
...
axs[4].plot(Tplot[:end] / DeltaT, x_exp_1[:end, Nx//2+16+12], 'r-')
axs[4].plot(Tplot[:end] / DeltaT, x_mhe_timeS[:end, Nx//2+16+12], 'g--')
axs[4].plot(Tplot[:end] / DeltaT, x_mhe_2[:end, Nx//2+16+12,:], 'b:')
...
plt.figure()
plt.plot(Tplot[:end] / DeltaT, RMSE_X, 'g--')
plt.plot(Tplot[:end] / DeltaT, RMSE_X_cmhe, 'b:')
plt.xticks(Tplot[:end:calNode * 24 * (3600/DeltaTmhe)] / DeltaT, np.arange(Tsim / (DeltaTmhe * (3600/DeltaTmhe) * 24) + 1, step=1))
plt.legend(['DMHE', 'CMHE'], loc=9, ncol=4, frameon=False)
...
plt.figure()
plt.plot(Tplot[5760:end] / DeltaT, RMSE_X[5760:end], 'g--')
plt.plot(Tplot[5760:end] / DeltaT, RMSE_X_cmhe[5760:end], 'b:')
plt.xticks(Tplot[5760:end:calNode * 24 * (3600/DeltaTmhe)] / DeltaT, np.array([4,5]))
# plt.legend(['DMHE', 'CMHE'], loc=9, ncol=4, frameon=False)
# plt.xlabel('Time, t (day)')
# plt.ylabel('RMSE')
plt.savefig('RMSE_zoomin.pdf')
plt.show()
